...
export default function MasterData() {
    ...
    const toggleAddNewProductOverlay = (e) => {
        if(document.querySelector('.tambah-barang-overlay').style.display == "block") {
            document.querySelector('.tambah-barang-overlay').style.display = "none";
        } else {
            document.querySelector('.tambah-barang-overlay').style.display = "block";
        }
    }
    ...
    const handleAddNewProduct = async (formData) => {
        ...
        const token = sessionStorage.getItem('jwt');
        try {
            const response = await fetch('http://localhost:3000/api/put/master-data/products', {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': token
            ...
            alert("Failed adding new products");
            console.log(err);
        }
    }
            ...
            <div className="daftar-semua-nama-barang">
                ...
                <table>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>
                                <img onClick={toggleAddNewProductOverlay} className='add-products' src="/Add box.svg" alt="Tambah Barang" />
                            </td>
                        </tr>
                    </tbody>
                </table>
                <div className="tambah-barang-overlay">
                    <div className="title-bar">
                        ...
                        <button onClick={toggleAddNewProductOverlay} className='close-button'>Tutup</button>
                    </div>
                    <hr />
                    <form action={handleAddNewProduct}>
                        <label htmlFor="product_name">Nama Produk</label>
                        <input type="text" name='product_name' placeholder='Masukan Nama Product Baru'/>
